...
USERNAME = os.environ.get("GITHUB_USERNAME", "DianaNerualNetwork")
...
# Entries may be bare names (looked up among owned repos) or "owner/repo".
FEATURED_REPOS = [
...
# Manual descriptions for repos whose GitHub "About" field is empty.
DESCRIPTION_OVERRIDES = {
    "PaddleDepth": "Lightweight, high-performance PaddlePaddle toolkit for depth "
    "information enhancement (completion, mono, stereo).",
}
...
def render_stats_svg(
    *,
    ...
    tiles = [
        ("Forks", total_forks, "#60A5FA"),
        ("Repos", public_repos, "#BFDBFE"),
        ("Followers", followers, "#7DD3FC"),
        ("PRs", prs, "#93C5FD"),
        ("Issues", issues, "#38BDF8"),
    ...
    for i, (label, value, color) in enumerate(side):
        ...
        tile_svg.append(
            ...
            f'<rect width="105" height="58" rx="12" fill="#0E1F4B" stroke="{color}" stroke-width="1.2" opacity="0">'
            f'<animate attributeName="opacity" from="0" to="1" dur="0.4s" begin="{delay:.2f}s" fill="freeze"/>'
            f"</rect>"
            f'<text x="12" y="22" fill="#B8CBE6" font-family="Segoe UI, Ubuntu, Sans-Serif" '
...
<defs>
  <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
    ...
    <stop offset="50%" stop-color="#0A1630"/>
    <stop offset="100%" stop-color="#0B1F45"/>
  </linearGradient>
...
<rect x="1" y="1" width="{width - 2}" height="{height - 2}" rx="17" fill="none" stroke="#3B82F6" stroke-opacity="0.55">
  <animate attributeName="stroke-opacity" values="0.35;0.85;0.35" dur="2.8s" repeatCount="indefinite"/>
</rect>

<text x="22" y="34" fill="#DBEAFE" font-family="Segoe UI, Ubuntu, Sans-Serif" font-size="15" font-weight="700">
  {escape(USERNAME)} · Research Dashboard
</text>
<text x="22" y="52" fill="#60A5FA" font-family="Segoe UI, Ubuntu, Sans-Serif" font-size="11" font-weight="600">
  Ocean Blue · Live GitHub Metrics
</text>

<!-- STAR CORE -->
<g transform="translate(20, 68)">
  <rect width="230" height="200" rx="16" fill="url(#panel)" stroke="#60A5FA" stroke-opacity="0.45"/>
  ...
  <circle cx="115" cy="100" r="62" fill="none" stroke="#3B82F6" stroke-width="3" stroke-linecap="round"
  ...
  {"".join(star_frames)}
  <text text-anchor="middle" x="115" y="192" fill="#DBEAFE" font-family="Segoe UI, Ubuntu, Sans-Serif"
...
def render_langs_svg(lang_counts: list[tuple[str, int]], limit: int = 8) -> str:
    ...
    for i, (lang, count) in enumerate(items):
        ...
        legend.append(
            ...
            f'<text x="264" y="{y}" fill="#DBEAFE" font-family="Segoe UI, Ubuntu, Sans-Serif" '
            ...
            f'<rect x="250" y="{y + 8}" width="140" height="5" rx="2.5" fill="#12306B"/>'
...
<rect x="1" y="1" width="{width - 2}" height="{height - 2}" rx="17" fill="none" stroke="#60A5FA" stroke-opacity="0.5">
...
{"".join(arcs)}
<circle cx="120" cy="150" r="46" fill="#081226"/>
<text text-anchor="middle" x="120" y="144" fill="#DBEAFE" font-family="Segoe UI, Ubuntu, Sans-Serif"
...
def render_repo_card(repo: dict[str, Any]) -> str:
    ...
    desc_lines = wrap_text(
        repo.get("description") or DESCRIPTION_OVERRIDES.get(name) or "",
        width=46,
        max_lines=2,
    )
...
<defs>
  <linearGradient id="pinStroke" x1="0" y1="0" x2="1" y2="1">
    ...
    <stop offset="100%" stop-color="{BLUE_SOFT}"/>
  </linearGradient>
</defs>
<style>
  .name {{ font: 700 15px 'Segoe UI', Ubuntu, Sans-Serif; fill: {BLUE}; }}
  .desc {{ font: 400 12px 'Segoe UI', Ubuntu, Sans-Serif; fill: {BLUE_MUTED}; }}
  .meta {{ font: 600 12px 'Segoe UI', Ubuntu, Sans-Serif; fill: {BLUE_INK}; }}
  .star-meta {{ font: 700 12px 'Segoe UI', Ubuntu, Sans-Serif; fill: #B8860B; }}
  .card {{ fill: #F6FAFF; stroke: url(#pinStroke); }}
...
<g class="card-body">
  ...
  {desc_svg}
  <circle cx="24" cy="96" r="4" fill="{BLUE_MID}"/>
...
def render_header_svg() -> str:
    ...
    for i, line in enumerate(lines):
        ...
        d = (i + 1) / n
        frames.append(
            f'<text text-anchor="middle" x="{width / 2:.0f}" y="118" fill="#DBEAFE" '
    ...
    return f"""<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" preserveAspectRatio="xMidYMid slice" role="img" aria-label="Bing Xiong">
<defs>
  ...
  <linearGradient id="waveFill" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%" stop-color="#050B1A" stop-opacity="0.15"/>
    <stop offset="100%" stop-color="#050B1A" stop-opacity="0.55"/>
  </linearGradient>
  <linearGradient id="nameGlow" x1="0" y1="0" x2="1" y2="0">
    <stop offset="0%" stop-color="#DBEAFE">
      <animate attributeName="stop-color" values="#DBEAFE;#E0F2FE;#BFDBFE;#93C5FD;#DBEAFE" dur="3s" repeatCount="indefinite"/>
    </stop>
    <stop offset="50%" stop-color="#EFF6FF">
      <animate attributeName="stop-color" values="#EFF6FF;#DBEAFE;#E0F2FE;#BFDBFE;#EFF6FF" dur="3s" repeatCount="indefinite"/>
    </stop>
    <stop offset="100%" stop-color="#93C5FD">
      <animate attributeName="stop-color" values="#93C5FD;#DBEAFE;#EFF6FF;#E0F2FE;#93C5FD" dur="3s" repeatCount="indefinite"/>
    </stop>
  </linearGradient>
</defs>
...
{"".join(frames)}
<text text-anchor="middle" x="{width / 2:.0f}" y="168" fill="#DBEAFE"
      font-family="Segoe UI, Ubuntu, Sans-Serif" font-size="14" font-weight="600">
  NUS · Computational Pathology · Virtual Staining · Medical Imaging
</text>
</svg>
"""
...
def render_about_card() -> str:
    ...
    chips = [
        ("Computational Pathology", 36, 118, 190),
        ("Virtual Staining", 242, 118, 140),
        ("Medical Imaging", 398, 118, 140),
    ]
    chip_svg = "".join(
        (
            f'<g>'
            f'<rect x="{x}" y="{y}" width="{w}" height="28" rx="14" fill="#EFF6FF"/>'
            f'<text x="{x + 12}" y="{y + 18}" fill="{BLUE_INK}" '
    ...
    return (
        ...
        f'viewBox="0 0 {width} {height}" role="img" aria-label="About Bing Xiong">'
        f"<defs>"
        f'<linearGradient id="aboutBg" x1="0" y1="0" x2="1" y2="0">'
        f'<stop offset="0%" stop-color="#0A1B3D"/>'
        f'<stop offset="50%" stop-color="{BLUE}"/>'
        f'<stop offset="100%" stop-color="{BLUE_MID}"/>'
        f"</linearGradient>"
        f"</defs>"
        f'<rect width="{width}" height="{height}" rx="14" fill="url(#aboutBg)"/>'
        f'<rect x="0" y="0" width="8" height="{height}" fill="#38BDF8"/>'
